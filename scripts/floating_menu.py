...
FLOATING=f'''<nav class="floating-contact" aria-label="상담 예약 오시는 길 빠른 메뉴"><button class="quick-toggle" type="button" aria-label="빠른 메뉴 접기" aria-expanded="true" aria-controls="floating-links" hidden>접기</button><div class="floating-links" id="floating-links"><a href="https://pf.kakao.com/_sYeKE" target="_blank" rel="noopener noreferrer" aria-label="카카오톡 상담">{chat}<span>상담</span></a><a href="https://m.booking.naver.com/booking/6/bizes/150330" target="_blank" rel="noopener noreferrer" aria-label="네이버 진료 예약">{calendar}<span>예약</span></a><a href="01_3.location.html" aria-label="오시는 길 안내">{pin}<span>오시는 길</span></a></div></nav>'''
ASSETS='<link rel="stylesheet" href="assets/floating.css"><script src="assets/floating.js" defer></script>'
