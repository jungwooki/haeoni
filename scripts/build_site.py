...
def link(url,label,cls=''):return f'<a class="{cls}" href="{esc(url,quote=True)}">{label}</a>'
depts=[('internal','한방내과','INTERNAL MEDICINE','증상을 넘어, 몸 전체의 흐름을 살핍니다.','호흡기 · 소화기 · 검사와 변증진단','/rhinitis'),('child','소아과','CHILD & ADOLESCENT','아이의 오늘과 건강한 성장을 함께 봅니다.','성장 · 허약체질 · 소아청소년 건강','/kids'),('women','부인과','WOMEN’S HEALTH','여성의 생애주기마다 세심하게 함께합니다.','여성질환 · 임신 준비 · 산후 회복','/woman'),('weight','비만','WEIGHT MANAGEMENT','생활과 몸의 변화를 함께 살핍니다.','체중 · 체형 · 생활 관리','https://haeonw.com'),('constitution','체질관리','CONSTITUTIONAL CARE','같은 증상도, 나의 몸에 맞게 살핍니다.','허약체질 · 보약 · 한약 처방','/152'),('pain','통증재활','PAIN & REHABILITATION','움직임의 회복에서 일상의 회복까지.','근골격 통증 · 재활 · 회복 관리','https://haeon.sportsmps.com/'),('sports','스포츠 MPS','SPORTS MPS','운동하는 몸과 마음을 함께 이해합니다.','유소년 스포츠 · 성인 운동 · MPS 평가','https://haeon.sportsmps.com/')]
...
visit_links=[('time.html','진료시간'),('01_3.location.html','오시는 길·주차'),('01_4.tour.html','공간 둘러보기'),('https://pf.kakao.com/_sYeKE','상담·예약'),('https://haeoni.com/link.cm?menu_code=m2025060560ee57bf82ead','비급여수가 안내')]
...
def header(current):
 parent='01_2.team.html' if current.startswith('doctor-') else 'departments.html' if current.startswith(('clinic-', 'internal-', 'child-', 'women-')) else 'time.html' if current in ['01_3.location.html','01_4.tour.html'] else '01_1.about.html' if current in ['history.html','contribution.html'] else current
 groups=''.join('<section><h2>'+title+'</h2>'+''.join(link(u,n) for u,n in items)+'</section>' for title,items in nav_groups)
 nav=''.join(link(u,n,'nav-current' if parent==u else '') for u,n in [('01_1.about.html','한의원 소개'),('departments.html','진료 분야'),('01_2.team.html','의료진'),('time.html','이용 안내')])
...
def write(name,title,content,group='한의원 소개',desc='본질을 지키며 미래로 향합니다.',hero=True,eyebrow='HAEON KOREAN MEDICINE'):
 ...
 group_url={'의료진':'01_2.team.html','진료 분야':'departments.html','이용 안내':'time.html','한방내과':'clinic-internal.html','소아과':'clinic-child.html','부인과':'clinic-women.html'}.get(group,'01_1.about.html')
 ...
 if name=='clinic-internal.html' or name.startswith('internal-'):
  html=html.replace('<body id="top">','<body id="top" class="internal-renewal">').replace('</head>','<link rel="stylesheet" href="assets/internal-renewal.css"><script src="assets/internal.js" defer></script></head>')
 if name in VISUAL_CONFIG:
...
def dept_cards():return '<div class="department-grid">'+''.join(f'<a class="department-card" href="clinic-{d[0]}.html"><span class="eyebrow">0{i+1} / {d[2]}</span><h3>{d[1]}</h3><p>{d[4]}</p><span aria-hidden="true">↗</span></a>' for i,d in enumerate(depts))+'</div>'
...
write('01_1.about.html','해온의 철학·인사말','<section class="section"><div class="container editorial-grid"><aside><p class="eyebrow">OUR PHILOSOPHY</p><h2>가업의 뜻과 경험,<br>마음의 아픔까지.</h2><img class="editorial-image" src="assets/images/original-hero-2.jpg" alt="해온한의원 진료실" width="1920" height="1080"></aside>'+'<div><p class="source-note">해온의 시작과 마음을 담은, 작성 당시의 인사말입니다.</p>'+prose(about['86'])+'</div>'+'</div></section>',desc='3대를 이어온 진료의 마음으로, 한의학을 가꾸어 갑니다.')
write('history.html','역사와 전통','<section class="section"><div class="container editorial-grid"><aside><p class="eyebrow">PAST · PRESENT · FUTURE</p><h2>깊은 뿌리에서<br>새로운 내일로.</h2><div class="milestones"><p><strong>3대</strong> 가업의 뜻과 경험</p><p><strong>2008</strong> 신도림에서 개원</p><p><strong>2017</strong> 강의와 학술교류</p><p><strong>2025</strong> 테크노마트 2층 확장</p></div></aside>'+'<div><p class="source-note">해온이 걸어온 길을 담은, 작성 당시의 기록입니다.</p>'+prose(about['87'])+'</div>'+'</div></section>',desc='원칙을 지키며 전통을 이어갑니다.')
...
for slug,name,role,area in docs:
 ...
 body='<p class="source-note">원장님의 인사말은 작성 당시의 경험과 생각을 담은 글입니다.</p>'+''.join(f'<section id="profile-{i}" class="profile-section"><h2>{h}</h2>'+prose(lines)+'</section>' for i,(h,lines) in enumerate(sections))
...
for slug,name,en,desc,area,source in depts:
 ...
 links=''.join(link(ROUTES.get(r['url'], 'https://haeoni.com'+r['url'] if r['url'].startswith('/') else r['url']),esc(r['title'])+' <span aria-hidden="true">↗</span>','topic-link') for r in items)
...
write('01_3.location.html','오시는 길·주차','''<section class="section"><div class="container editorial-grid"><aside><p class="eyebrow">LOCATION</p><h2>신도림역과 연결된<br>테크노마트 2층.</h2><p class="aside-copy">서울시 구로구 새말로 97<br>신도림 테크노마트 2층 9호</p><a class="underlined-link" href="https://m.place.naver.com/hospital/13108422/home" target="_blank" rel="noopener noreferrer">네이버 지도에서 보기 ↗</a></aside><div class="prose"><h2>지하철로 오시는 길</h2><p>1·2호선 신도림역 3번 출구 지하통로를 통해 테크노마트로 들어오세요. 엘리베이터 또는 에스컬레이터로 2층으로 올라오시면 됩니다.</p><h2>주차 안내</h2><p>테크노마트 지하 3층부터 지하 7층까지 주차장을 이용하실 수 있습니다. 주차는 3시간 무료입니다.</p><p>토요일은 주변 도로가 혼잡할 수 있으니 예약 시간보다 조금 일찍 도착해 주세요.</p><h2>2008년부터 신도림에서</h2><p>해온한의원은 2008년부터 신도림 테크노마트에서 진료하고 있습니다. 2025년 2층 확장공사를 통해 더욱 많은 분들께 세밀한 진료로 보답합니다.</p><a class="underlined-link" href="time.html">진료시간 확인 →</a></div></div></section>''',group='이용 안내',desc='1·2호선 신도림역 지하통로로 편하게 오세요.',eyebrow='LOCATION & PARKING')
